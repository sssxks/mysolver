...
impl EGraph {
    ...
    fn collect_affected_parents(&mut self, absorbed_root: ENodeId) -> Vec<ENodeId> {
        ...
        let mut result = Vec::new();
        for member in self.class_members(absorbed_root) {
            for &parent in &self.enodes[member.index()].parents {
    ...
    fn explain_edge(&self, edge: EqEdgeId, out: &mut HashSet<Lit>, active: &mut HashSet<EqKey>) {
